...
e1000_transmit(struct mbuf *m)
{
  ...
  */
  ...
  /*Otherwise, use mbuffree() to free 
  * the last mbuf that was transmitted from that descriptor
  * (if there was one). 
  */
  if (tx_mbufs[index] != 0) {
...
e1000_recv(void)
{
  ...
  {
    /*
  * First ask the E1000 for the ring index at which 
  * the next waiting received packet (if any) is located, 
  * by fetching the E1000_RDT control register and 
  ...
  */
    ...
    **/
    ...
    /*
    Otherwise, update the mbuf's m->len to the length 
    reported in the descriptor. Deliver the mbuf to the network 
    stack using net_rx(). 
    */
    rx_mbufs[index]->len = desc->length;
    net_rx(rx_mbufs[index]);
    /*Then allocate a new mbuf using mbufalloc()
